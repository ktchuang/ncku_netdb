...
public class MQTurnAroundPNewNearbyV1Cache {
	//static int num_k = 5;
	ArrayList<Integer> queryList;
	static TreeSet<VisitedObject> remainingQ;
	static HashSet<Integer> tmpN = new HashSet<Integer>();
	static HashSet<Integer> tmpO = new HashSet<Integer>();
	...
	public static void main(String[] args) throws IOException {
    	...
    	int cacheSize = Integer.valueOf(args[9]);
    	int intputSize = Integer.valueOf(args[10]);
		MQTurnAroundPNewNearbyV1Cache upq = new MQTurnAroundPNewNearbyV1Cache(args[0],args[1],args[2],args[3]);
		...
		ArrayList<Integer> cacheRes = new ArrayList<Integer>();
		HashSet<Integer> cacheIdx = new HashSet<Integer>();
		...
		while (upq.queryList.size()>0) {
    		...
    		else if (cacheIdx.contains(oriVID)) {
    			upq.queryList.remove(0);
        		arrivalT.remove(0);
    			ttt++;
    			if (arrivalT.get(0)<currT) {
    				turnAroundT += (currT-arrivalT.get(0));
    			}
    			continue;
    		}
    		...
    		if (cacheFlag==1) {
    			if  (cacheRes.size()<cacheSize) {
    				cacheRes.add(oriVID);
    				cacheIdx.add(oriVID);
    			}
    			else {
    				int t = cacheRes.remove(0);
    				cacheIdx.remove(t);
    				cacheRes.add(oriVID);
    				cacheIdx.add(oriVID);
    			}
    		...
    		HashMap<Integer,HashSet<Integer>> checkVIDHash = new HashMap<Integer,HashSet<Integer>>();
    		double currAT = arrivalT.get(0);
    		upq.queryList.remove(0);
    		arrivalT.remove(0);
    		int tmpIdx=0; 
    		double tmpMax=0;
    		
    		//the same query point && neighboring points
    		v = GraphPool.getSignleton().getVertex(oriVID);
    		List<Integer> neighbors = v.getNeighbors();
			HashSet<Integer> neighborSet = new HashSet<Integer>();
			for (int i=0; i<neighbors.size(); i++) {
				neighborSet.add(neighbors.get(i));
			}
			int reuse = 0;
    		while ((arrivalT.size()>0)&&(tmpIdx<arrivalT.size())&&(arrivalT.get(tmpIdx)<=currT)) {
    			//System.out.println(arrivalT.size()+" "+tmpIdx+" "+ttt);
    			int vID = upq.queryList.get(tmpIdx);
    			if (oriVID == vID) {
    				upq.queryList.set(tmpIdx, -1);
    				//upq.queryList.remove(tmpIdx);
    				//arrivalT.remove(tmpIdx);
    				tmpIdx++;
    				reuse++;
    				ttt++;
    			}
    			else {
    				if (neighborSet.contains(vID)) {
    					Edge e = GraphPool.getSignleton().getEdge(oriVID, vID);
    					if (checkVIDHash.containsKey(vID)) {
    						HashSet<Integer> tmpVIDSet = checkVIDHash.get(vID);
    						tmpVIDSet.add(tmpIdx);
    						checkVIDHash.put(vID, tmpVIDSet);
    					}
    					else {
    						HashSet<Integer> vIDSet = new HashSet<Integer>();
    						vIDSet.add(tmpIdx);
    						checkVIDHash.put(vID, vIDSet);
    					}
    					if (e.getWeight()>tmpMax)
    						tmpMax = e.getWeight();
    				}
    				tmpIdx++;
    			}
    		}
    		timer.setNanoStartTime("Checking");
    		int objFind = 0;
    		int reuseCnt = 0;
    		double total = 0;
        	...
        	double newExpDist = results.get(num_k-1).getDist()+tmpMax*2;
        	
    		while (remainingQ.size()>0 && objFind==0) {
    			VisitedObject popObj = remainingQ.pollFirst();
    			if ((popObj.getDist()>=newExpDist)&&(objFind==0)) {
    				//remove query
    				Iterator<Integer> iter = checkVIDHash.keySet().iterator();
    				while (iter.hasNext()) {
    	    			int key = (Integer)iter.next();
    	    			HashSet<Integer> set = checkVIDHash.get(key);
    	    			Iterator<Integer> iiter = set.iterator();
    	    			while (iiter.hasNext()) {
        	    			int idx = (Integer)iiter.next();
        	    			upq.queryList.set(idx, -1);
        	    			double waitingT = currT-arrivalT.get(idx);
        	    			total += waitingT;
        	    			if (waitingT>maxTAT)
        	    				maxTAT = waitingT;
        	    			if (waitingT<minTAT)
    			...
    			if ((tmpN.contains(popObj.getVID()))||(tmpO.contains(popObj.getMsgID()))) {
    				continue;
    			}
    			if (popObj.getVID()>=0) {  //pop up object is a node
    				...
    				objFind = 1;
    				tmpO.add(popObj.getMsgID());
    				double distTH = (popObj.getDist()-results.get(num_k-1).getDist())/2;
    					
    				Iterator<Integer> iter = checkVIDHash.keySet().iterator();
        			while (iter.hasNext()) {
        	    		int key = (Integer)iter.next();
        	    		Edge e = GraphPool.getSignleton().getEdge(oriVID, key);
        	    		if (e.getWeight()<distTH) {
        	    			HashSet<Integer> set = checkVIDHash.get(key);
        	    			Iterator<Integer> iiter = set.iterator();
        	    			while (iiter.hasNext()) {
        	    				int idx = (Integer)iiter.next();
    		...
    		double extraExecTime = timer.setNanoEndTime().getMillisSeconds();
    		currT += extraExecTime;
    		totalExecT += extraExecTime; 
    		turnAroundT += (extraExecTime*reuseCnt);
    		double newT = execTime+extraExecTime;
    		//if((reuse>0)||(reuseCnt>0))
    		//out.write(execTime+" "+newT+" "+reuse+" "+reuseCnt+"\n");
    		if (currAT==prevArrT) {
    			averageTAT += printTAT;
    			averageTAT += total;
    			averageTAT += (extraExecTime*reuseCnt);
    	...
    	System.out.println("equal count:"+ttt+" "+vvv+" "+xxx);
    	/*
	...
	public MQTurnAroundPNewNearbyV1Cache(String networkFn, String hierFn, String msgFn, String queryFn) throws IOException {
