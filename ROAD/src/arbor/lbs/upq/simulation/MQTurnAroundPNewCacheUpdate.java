...
public class MQTurnAroundPNewCacheUpdate {
	//static int num_k = 5;
	...
	static HashMap<Integer, ArrayList<VisitedObject>> cacheKnnMap = new HashMap<Integer, ArrayList<VisitedObject>>() ;
	...
	public static void main(String[] args) throws IOException {
		...
		int updateFreq = Integer.valueOf(args[8]);
    	int cacheSize = Integer.valueOf(args[9]);
    	int intputSize = Integer.valueOf(args[10]);
		MQTurnAroundPNewCacheUpdate upq = new MQTurnAroundPNewCacheUpdate(args[0],args[1],args[2],args[3]);
		...
		*/
		...
		while (upq.queryList.size()>0) {
    		...
    		ArrayList<VisitedObject> results = new ArrayList<VisitedObject>();
    		if (cacheIdx.contains(oriVID)) {
    			ttt++;
    		}
    		else {
    			results = upq.getKNN(oriVID,num_k);
    		...
    		if (cacheFlag==1) {
    			if  (cacheRes.size()<cacheSize) {
    				cacheRes.add(oriVID);
    				cacheIdx.add(oriVID);
    				cacheKnnMap.put(oriVID, results);
    				//for update
    				Vertex vert = GraphPool.getSignleton().getVertex(oriVID);
    	    		Point loc = vert.getloc();
    				tree.insertData(oriVID, loc.getCoord(0), loc.getCoord(1));
    				int size = results.size();
        			VisitedObject kthObj = results.get(size-1);
    				...
    				cacheIdx.remove(t);
    				cacheKnnMap.remove(t);
    				//for update
    				Vertex vert = GraphPool.getSignleton().getVertex(t);
    	    		Point loc = vert.getloc();
    				tree.deleteData(t, loc.getCoord(0), loc.getCoord(1));
    				cacheRes.add(oriVID);
    				cacheIdx.add(oriVID);
    				cacheKnnMap.put(oriVID, results);
    				//for update
    				vert = GraphPool.getSignleton().getVertex(oriVID);
    	    		loc = vert.getloc();
    				tree.insertData(oriVID, loc.getCoord(0), loc.getCoord(1));
    				int size = results.size();
        			VisitedObject kthObj = results.get(size-1);
    		...
    		double updateTime = 0;
    		if (l%updateFreq==0) {
    			List<Vertex> allVertex = GraphPool.getSignleton().getAllVertexes();
    			int randomInt = (int)(Math.random()*(allVertex.size()-1));
    			Vertex randomVertex = GraphPool.getSignleton().getVertex(randomInt);    	
    			
    			upq.insertObject(randomVertex.getId());
    			double[] xy = new double[2];
    			xy[0] = randomVertex.getloc().getCoord(0);
    			xy[1] = randomVertex.getloc().getCoord(1);
    			Point insertObj = new Point(xy);
    			timer.setNanoStartTime("Update Performance");
    			HashSet<Integer> updatedCaches = upq.GetUpdatedBorder(insertObj, maxKDist);
    			Iterator<Integer> cacheIter = updatedCaches.iterator();
    			while (cacheIter.hasNext()) {
    				int removeID = (Integer)cacheIter.next();
    				cacheIdx.remove(removeID);
    				int idx = cacheRes.indexOf(removeID);
    				if (idx!=-1) {
    					cacheRes.remove(idx);
    				}
    				removeCnt++;
    			}
    			updateTime = timer.setNanoEndTime().getMillisSeconds();
    		}
    	...
    	System.out.println("equal count:"+ttt+" "+removeCnt);
    	/*
	...
	public HashSet<Integer> GetUpdatedBorder(Point insertObj, double deltaLen) {
		...
		while (candidates.size()>0) {
			...
			for (int i=0; i<candidates.size(); i++) {
				...
				ArrayList<VisitedObject> preKNN = cacheKnnMap.get(v.getId());
				if (preKNN==null) {
	...
	public MQTurnAroundPNewCacheUpdate(String networkFn, String hierFn, String msgFn, String queryFn) throws IOException {
