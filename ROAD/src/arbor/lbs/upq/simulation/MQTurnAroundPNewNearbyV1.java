...
public class MQTurnAroundPNewNearbyV1 {
	//static int num_k = 5;
	...
	public static void main(String[] args) throws IOException {
		...
		MQTurnAroundPNewNearbyV1 upq = new MQTurnAroundPNewNearbyV1(args[0],args[1],args[2],args[3]);
		...
		while (upq.queryList.size()>0) {
    		...
    		while ((arrivalT.size()>0)&&(tmpIdx<arrivalT.size())&&(arrivalT.get(tmpIdx)<=currT)) {
    			...
    			if (oriVID == vID) {
    				...
    				double waitingT = currT-arrivalT.get(tmpIdx);
    				total += waitingT;
    				if (waitingT>maxTAT)
	    				maxTAT = waitingT;
	    			if (waitingT<minTAT)
	...
	public MQTurnAroundPNewNearbyV1(String networkFn, String hierFn, String msgFn, String queryFn) throws IOException {
