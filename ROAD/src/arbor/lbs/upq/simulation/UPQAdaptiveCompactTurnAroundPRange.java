...
public class UPQAdaptiveCompactTurnAroundPRange {
	//static int num_k = 5;
	...
	public static void main(String[] args) throws IOException {
		...
		UPQAdaptiveCompactTurnAroundPRange upq = new UPQAdaptiveCompactTurnAroundPRange(args[0],args[1],args[2],args[3]);
		...
		for (int l=0; l<intputSize; l++) {
    		...
    		if (l%1000==999) {
    			//out.write("UPQ Compact Turn Around Time:"+turnAroundT+"\n");
	...
	public UPQAdaptiveCompactTurnAroundPRange(String networkFn, String hierFn, String msgFn, String queryFn) throws IOException {
   ...
   /* 
	...
	private void ChooseSearchPathNew(TreeSet<VisitedObject> queue, Vertex popVertex,
    	...
    	ChoosePath(queue,popVertex,dist,visitedN,visitedO,k);
	}
    ...
    public ArrayList<VisitedObject> getRoadKNN(int srcVID, int k) throws IOException {
    	...
    	while (queue.size()>0) {
    		...
    		if (popObj.getVID()>=0) {  //pop up object is a node
    			...
    			ChoosePath(queue,popVertex,popObj.getDist(),visitedN,visitedO,k);
    			expNodeCnt++;
    			visitedN.add(popObj.getVID());
    		}
    		else { //pop up object is a msgObj
