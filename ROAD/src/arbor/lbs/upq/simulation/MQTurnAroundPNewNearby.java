...
public class MQTurnAroundPNewNearby {
	//static int num_k = 5;
	...
	public static void main(String[] args) throws IOException {
		...
		MQTurnAroundPNewNearby upq = new MQTurnAroundPNewNearby(args[0],args[1],args[2],args[3]);
		...
		*/
		...
		while (upq.queryList.size()>0) {
    		...
    		int oriLeafRID = v.getLeafRnetID();
    		...
    		TreeSet<VisitedObject> checkVIDSet = new TreeSet<VisitedObject>();
    		upq.queryList.remove(0);
    		arrivalT.remove(0);
    		int tmpIdx=0; 
    		...
    		while ((arrivalT.size()>0)&&(tmpIdx<arrivalT.size())&&(arrivalT.get(tmpIdx)<=currT)) {
    			...
    			if (oriVID == vID) {
    				...
    				if (neighborSet.contains(vID)) {
    					...
    					VisitedObject obj = new VisitedObject(vID,tmpIdx,e.getWeight()); 
    					checkVIDSet.add(obj);
    				}
    				tmpIdx++;
    			}
    		...
    		int objFind = 0;
    		while (checkVIDSet.size()>0 && objFind==0) {
    			
    			VisitedObject popQuery = checkVIDSet.pollFirst();
        		double newExpDist = results.get(num_k-1).getDist()+popQuery.getDist()*2;
        		
    			while (remainingQ.size()>0 && objFind==0) {
    				VisitedObject popObj = remainingQ.pollFirst();
    				if ((popObj.getDist()>=newExpDist)&&(objFind==0)) {
    					//remove query
    					int queryIdx = popQuery.getMsgID();
    					upq.queryList.set(queryIdx, -1);
    					vvv++;
    					break;
    				}
	...
	public MQTurnAroundPNewNearby(String networkFn, String hierFn, String msgFn, String queryFn) throws IOException {
